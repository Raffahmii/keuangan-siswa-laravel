    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 max-w-2xl transition-all duration-500 hover:shadow-2xl">
        <form method="POST" action="{{ route('transactions.store') }}">
            ...
            <div class="mb-4">
                ...
                <input type="number" name="amount" value="{{ old('amount') }}" class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg shadow-sm focus:ring-teal-500 focus:border-teal-500">
                @error('amount') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
            </div>

            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Deskripsi</label>
                <input type="text" name="description" value="{{ old('description') }}" class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg shadow-sm focus:ring-teal-500 focus:border-teal-500" placeholder="Opsional">
                @error('description') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
            </div>

            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Tanggal</label>
                <input type="date" name="date" value="{{ old('date') }}" class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg shadow-sm focus:ring-teal-500 focus:border-teal-500">
